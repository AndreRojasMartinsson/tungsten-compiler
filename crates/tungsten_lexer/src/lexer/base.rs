...
impl<'a> Lexer<'a> {
    ...
    pub(crate) fn report_error(&mut self, err: LexerError, span: Range<usize>) {
        match err {
            ...
            LexerError::UnterminatedComment => {
                self.context
                    .add_error(error_builders::build_unterminated_comment_error(span));
            }
    ...
    fn read_identifier(&mut self, initial_char: char) -> (Kind, Option<Value>) {
        ...
        match value.as_ref() {
            ...
            "void" | "nil" | "uint" | "int" | "bool" | "str" | "float" => {
                (Kind::PrimitiveType, Some(Value::String(atom!(value))))
            }
